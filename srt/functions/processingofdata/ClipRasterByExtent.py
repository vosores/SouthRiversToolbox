...
pluginPath = os.path.split(os.path.split(os.path.split(os.path.realpath(__file__))[0])[0])[0]


class ClipRasterByExtent(AlgorithmMetadata, GdalAlgorithm):
    
    METADATA = AlgorithmMetadata.read(__file__, 'ClipRasterByExtent')
    INPUT = 'INPUT'
    EXTENT = 'PROJWIN'
    NODATA = 'NODATA'
    OPTIONS = 'OPTIONS'
    ...
    def icon(self):
        return QIcon(os.path.join(pluginPath, 'images', 'raster-clip.png'))
